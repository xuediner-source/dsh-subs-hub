...
function withThoughtSignature(mapped, part) {
	const signature = antigravityThoughtSignature(part);
...
function antigravityContentPart(part, callNamesById) {
	...
	if (part?.type === "tool-call") {
		...
		return withThoughtSignature({ functionCall: { name, args: parseAntigravityToolArgs(part.arguments) } }, part);
	}
...
class AntigravityStreamTranslator {
	...
	close(kind) {
		...
		if (kind === "tool-call") {
			assembled = {
				type: "tool-call",
				id: CallId(block.callId.length > 0 ? block.callId : `call_${block.index}`),
				name: block.name,
