...
function registerAuthRpc(ctx, controller, speed) {
	// `rpc.handle` mounts the channel as a webserver route through the CALLING
	// context (dsh-client-connection reads `owner.webServer`), so this context
	// must inject webServer itself. Scoping it here — rather than in the
	// top-level inject list — keeps the LLM adapters loadable in headless
	// profiles that have no webServer.
